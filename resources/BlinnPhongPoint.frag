...
void main(void)
{
    ...
    vec3 diffuseAlbedo = vec3(materialDiffuse) *
    vec3(lightSourceDiffuse);
    vec3 specularAlbedo = vec3(materialSpecular) *
    vec3(lightSourceSpecular);
    ...
    vec3 specular = pow(max(dot(N, H), 0.0), materialShininess) *
    specularAlbedo;
